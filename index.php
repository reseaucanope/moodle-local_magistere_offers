...
$PAGE->set_url('/local/magistere_offers/index.php', array('v' => $display));
...
$currentUrl = (new moodle_url('/local/magistere_offers/index.php', array('v' => $display)))->out();
...
$PAGE->requires->js_call_amd("local_magistere_offers/offers", "easyPaginate");
...
switch ($display) {
    ...
    default:
        if (OfferCourse::isIndexationAvailable()){
            ...
            $publics = new PublicsByFunctionUser($USER->id, $display);
            $courseForm = new course_form($currentUrl, array('v'=>$display, 'publics'=>$publics->prepare_publics_for_checkboxes_form()));
        }else{
            $courseForm = new course_form($currentUrl, array('v'=>$display));
        }
    ...
    if (OfferCourse::isMag()){
        ...
        echo html_writer::div(
            html_writer::tag('h1', get_string('title_parcours','local_magistere_offers')).
            html_writer::tag('div', get_string('desc_parcours','local_magistere_offers')),
            'headertitle'
        );
    ...
    case VIEW_FORMATION:
        if (OfferCourse::isIndexationAvailable()){
            ...
            if($USER && $USER->id > 0){
                $publics = new PublicsByFunctionUser($USER->id, $display);
                $formationForm = new formation_form($currentUrl, ['v'=>$display,
            ...
            $formationForm = new formation_form($currentUrl, ['v'=>$display, 'search_name'=>$search_result]);
        }
        ...
        echo html_writer::div(
            html_writer::tag('h1', get_string('title_formation','local_magistere_offers')).
            html_writer::tag('div', get_string('desc_formation','local_magistere_offers')),
            'headertitle'
        );
