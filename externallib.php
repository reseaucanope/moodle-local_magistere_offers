    ...
    public static function get_detail_offer($id) {
        ...
        if($offer) {
            ...
            $data_array = array(
                ...
                'description' => format_text($offer->summary),
                'is_formation' => $is_formation,
                'pdf_url' => null
            );

            if ($USER->id > 0 && $USER->auth == 'shibboleth') {
            ...
            if (OfferCourse::isIndexationAvailable()) {
                ...
                $data_array['objective'] = format_text($offer->objectif);
            ...
            if($is_formation) {
                ...
                if (($USER->id > 0 && $USER->auth == 'shibboleth')
                    || ($USER->id > 0 && $USER->auth == 'manual' && $CFG->academie_name != 'frontal' && $CFG->academie_name == $offer->aca_uri)) {
                ...
                if (isset($CFG->academie_name)) {
                    ...
                    if ($USER->id > 0 && $USER->auth == 'shibboleth' && $mainaca != false
                        ...
                        && $USER->auth == 'manual'
                        && $CFG->academie_name != 'frontal'
                        && FrontalFeatures::has_capability($CFG->academie_name, 'moodle/restore:restorecourse', context_system::instance()->id)) {
                        $restore_url = new moodle_url($CFG->wwwroot."/local/coursehub/restore.php", $params);
                        $data_array['restore_url'] = $restore_url->out(false);
