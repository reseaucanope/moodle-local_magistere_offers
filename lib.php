...
class OfferCourse{
    ...
    protected function build_data_list($list){
        foreach($list as $ind){
            ...
            if($this->tab == 'formation'){
                $key .= (isset($ind->startdate)? date('YmdHis',$ind->startdate):'_0');
    ...
    public function get_course_offers(){
        ...
        if(OfferCourse::isIndexationAvailable() && $USER->id != 0 && isloggedin()){
            $publics_favorite = html_writer::tag('button',
    ...
    private function generate_export_link($userid){
        $pdf_url = new moodle_url('/local/magistere_offers/pdfCatalog/pdfCatalog.php', array(
            'gen' => 'true',
            'filter' => base64_encode(serialize($this->filters)),
            'tab' => $this->tab,
            'offers_count' => count($this->offers),
            'userid' => $userid
        ));
        $xml_url = new moodle_url('/local/magistere_offers/xmlCatalog.php', array(
            'filter' => base64_encode(serialize($this->filters)),
            'tab' => $this->tab,
            'userid' => $userid
        ));
        $pdf_href = html_writer::link($pdf_url,
            html_writer::tag('i','',array('class'=>'fa fa-file-pdf-o'))
            . get_string('pdf_catalogue_link', 'local_magistere_offers'), array('class'=> 'dropdown-item pdf-catalog')
        );
        $xml_href = html_writer::link($xml_url,
            html_writer::tag('i','',array('class'=>'fa fa-file-export fa-file-code-o'))
            . get_string('xml_catalogue_link', 'local_magistere_offers'), array('class'=> 'dropdown-item xml-catalog')
        );
...
//            $carret = html_writer::tag('i','',array('class'=>'fa fa-caret-down'));
        ...
        $menu_href = html_writer::link('', get_string('export_catalogue_link', 'local_magistere_offers') . $carret , array(
    ...
    protected function get_course_offers_sql($dbconn, $indexData = null, $tab, $notification = false){
        ...
        if ($this->tab == 'formation') {
            ...
            if ($this->userid > 0 || $USER->id > 0) {
                $aca_shortname = $this->mainaca;
                if ($USER->id > 0) {
                    // Ajout d'une condition pour les parcours locaux pour cibler les offres sur le domaine uniquement
                    if ($USER->auth == 'manual' && $CFG->academie_name != 'frontal') {
                        $is_local_user = true;
                        $aca_shortname = $CFG->academie_name;
                    }
                } else {
                    if((databaseConnection::instance()->get($CFG->academie_name)) === false){exit;}
                    $user = databaseConnection::instance()->get($CFG->academie_name)->get_record('user', ['id' => $this->userid]);
                    if ($user->auth == 'manual' && $CFG->academie_name != 'frontal') {
                        $is_local_user = true;
                        $aca_shortname = $CFG->academie_name;
                    }
                ...
                if ($is_local_user) {
                    $sql_filter .= 'AND ((lcc.isasession = 1 OR lcc.isalocalsession = 1) AND lcs.identifiant = "'.$aca_shortname.'") ';
                } else {
                    $sql_filter .= 'AND (lcc.isasession = 1 
                                        OR (lcc.isalocalsession = 1 AND lcs.identifiant = "'.$aca_shortname.'")) ';
                }
            } else {
    ...
    static function string_format_origine_offers($academie){
        ...
        if(strpos($academie, 'ac-') === 0){
            ...
            $academiename = get_string('origin_academie_label', 'local_magistere_offers', $academiename);
        }else{
    ...
    static private function get_remotehub_course_offer($id){
        ...
        $course->course_demourl = $course->course_url;
...
WHERE ra.roleid=? AND ra.userid=?', array($role->id, $userid));
    ...
    public function dialog_restore_course(){
        ...
        if ($USER->id == 0 && OfferCourse::user_has_specific_loggedin('efe')) {
            ...
            if ($USER->id > 0 && $USER->auth == 'manual' && $CFG->academie_name != 'frontal') {
                $nexturl = new moodle_url("/local/coursehub/restore.php");
            } else {
    ...
    private function subcategory_select_content(){
        ...
        $content = '<option value="'.$main_category->id.'">'
            .get_string('nosubcategory', 'local_magistere_offers').'</option>';
        if($subcategory_tree){
...
class OfferCourseXML{
    ...
    private function construct_xml_contain(){
        ...
        foreach($this->offers as $offer){
            ...
            $formation->addChild('url', $offer->course_url);
        }
...
class PublicsByFunctionUser{
    ...
    public function create_modal($modal_name){
        ...
        $html ='<div class="modal fade publics-modal" style="margin-left: 0;" id="'.$modal_name.'" tabindex="-1" role="dialog" aria-labelledby="'.$modal_name.'" aria-hidden="true">
...
class NotificationNewCourses{
    ...
    public function notification_cron(){
        ...
        if($this->users && count($this->users) > 0){
            ...
            foreach ($this->users as $userto){
                ...
                if (empty($publicids)) { // Si aucun public coché, tous les publics sont sélectionnés
                    $publicids = implode(',', array_keys(OfferCourse::get_indexation_publics()));
                }
                ...
                if($publicids){
                    ...
                    $courses = $this->get_courses($filters, $userto->id);
     ...
     * @return array
     */
    private function get_courses($filters = null, $userid){
        $courses = new OfferCourse($filters, $this->tab, $userid,true);
        return $courses->get_all_course_offers();
    }

    /**
     * Fonction qui récupère l'ensemble des utilisateurs souhaitant obtenir une notification répondant aux cas suivants:
     *    - pour les comptes shibboleth, leur academie principale correspond à l'académie qui execute le cron d'envoi 
     *    - pour les comptes locaux, pas besoin de controler leur academie principale
     * @return array
     * @throws dml_exception
     */
    private function get_all_users_with_notification_allowed(){
        ...
        AND uid.fieldid = (SELECT id FROM {user_info_field} WHERE shortname = 'mainacademy')
        AND uid.data = ? AND u.auth = ?
        UNION
        SELECT u.* FROM {user} as u 
        LEFT JOIN {user_preferences} up ON (up.userid = u.id)
        WHERE up.name LIKE '".$name_user_pref."' AND up.value = '1' AND u.auth != ?
        ", [$CFG->academie_name, 'shibboleth', 'shibboleth']
        );
    }
    ...
    private function notification_make_mail_text($userto, $courses, $publicids){
        ...
        foreach($courses as $course){
            $offer = OfferCourse::get_hub_course_offer($course->publishid);
            $is_formation = false;
            if ($offer->publish == CourseHub::PUBLISH_PUBLISHED){
                $is_formation = true;
            }
            $offer_url = (new moodle_url($CFG->magistere_domaine.'/'.($is_formation?'f':'p').$offer->fakeid))->out();

            $title = '- '.$course->fullname.' ('.$offer_url.') '."\n".'';
            if ($this->tab == "course") {
                if (isset($course->course_demourl) && $course->course_demourl != null) {
                    $title = '- '.$course->fullname.' ('.$offer_url.') (Parcours en démonstration) '."\n".'';
                }
            }
            $courses_list .= $title;
        }
        ...
        $link_preference = get_string('email_message_preference_link', 'local_magistere_offers')
            .'('.$CFG->wwwroot.'/local/magistere_offers/index.php?v=parcours&action=changepref)'."\n";
        if ($this->tab == 'formation') {
            $link_preference = get_string('email_message_preference_link', 'local_magistere_offers')
                .'('.$CFG->wwwroot.'/local/magistere_offers/index.php?v=formation&action=changepref)'."\n";
        }
    ...
    private function notification_make_mail_html($userto, $courses, $publicids) {
        ...
        foreach($courses as $course){
            ...
            if ($offer) {
                ...
                if ($this->tab == "course") {
                    if (isset($course->course_demourl) && $course->course_demourl != null) {
                        $title = $course->fullname.' (Parcours en démonstration)';
                    }
                }
                $courses_list .= '<a target="_blank" href="'.$offer_url.'">'.$title.'</a>';
                $courses_list .= '</li>';
            }
        }
        $courses_list .= '</ul>';
        ...
        $link_preference = '<a target="_blank" href="'.$CFG->wwwroot.'/local/magistere_offers/index.php?v=parcours&action=changepref">'
            .get_string('email_message_preference_link', 'local_magistere_offers').'</a>';
        if ($this->tab == 'formation') {
            $link_preference = '<a target="_blank" href="'.$CFG->wwwroot.'/local/magistere_offers/index.php?v=formation&action=changepref">'
                .get_string('email_message_preference_link', 'local_magistere_offers').'</a>';
        }
